 ...
 */
uint8_t bme68x_i2c_read(uint8_t reg_addr, uint8_t *reg_data, uint32_t len,
                        void *intf_ptr) {
 ...
 */
uint8_t bme68x_i2c_write(uint8_t reg_addr, uint8_t *reg_data, uint32_t len,
                         void *intf_ptr) {
  uint8_t rslt = 0; /* Return 0 for Success, non-zero for failure */
...
bme680_bsec_update_subscription(float sample_rate) {
  ...
  requested_virtual_sensors[1].sensor_id =
      BSEC_OUTPUT_SENSOR_HEAT_COMPENSATED_TEMPERATURE;
  requested_virtual_sensors[1].sample_rate = sample_rate;
  requested_virtual_sensors[2].sensor_id = BSEC_OUTPUT_RAW_PRESSURE;
  requested_virtual_sensors[2].sample_rate = sample_rate;
  requested_virtual_sensors[3].sensor_id =
...
int8_t iot_bsec_process_data(int64_t current_time_ns,
  ...
  if (BSEC_CHECK_INPUT(sensor_settings->process_data,
                       BSEC_INPUT_PROFILE_PART) &&
      (data->status & BME68X_GASM_VALID_MSK)) {
    inputs[num_bsec_inputs].sensor_id = BSEC_INPUT_PROFILE_PART;
    inputs[num_bsec_inputs].signal =
        (operating_mode == BME68X_FORCED_MODE) ? 0 : data->gas_index;
    inputs[num_bsec_inputs].time_stamp = current_time_ns;
    num_bsec_inputs++;
  }
  ...
  if (num_bsec_inputs > 0) {
    ...
    status = bsec_do_steps(inputs, num_bsec_inputs, &bsec_outputs,
                           &num_bsec_outputs);
...
int8_t iot_init_bme68x(struct bme68x_dev *bme, struct bme68x_conf *conf,
...
int8_t iot_update_bme68x_config(struct bme68x_dev *bme,
...
int main() {
  ...
  while (1) {
    ...
    if (sensor_settings.trigger_measurement) {
      rslt =
          iot_update_bme68x_config(&bme, &conf, &heatr_conf, &sensor_settings);
      if (rslt != BME68X_OK) {
        return rslt;
      }

      // /* Calculate delay period in microseconds */
      del_period = bme68x_get_meas_dur(sensor_settings.op_mode, &conf, &bme) +
      ...
      if (n_fields) {
        ...
        rslt = iot_bsec_process_data(current_time_ns, &sensor_settings, &data,
                                     &processed_data);
        if (rslt != BSEC_OK) {
        ...
        j = sprintf(buffer, "{");
        j += sprintf(buffer + j, "\"ts\":%lld",
                     processed_data.timestamp / 1000000);
        j += sprintf(buffer + j, ",\"temp\":%.2f", processed_data.temperature);
        j += sprintf(buffer + j, ",\"pressure\":%.2f",
                     processed_data.raw_pressure);
        j += sprintf(buffer + j, ",\"humidity\":%.2f", processed_data.humidity);
        j += sprintf(buffer + j, ",\"raw_gas\":%.2f", processed_data.raw_gas);

        j += sprintf(buffer + j, ",\"co2_equivalent\":%.2f",
                     processed_data.co2_equivalent);
        j += sprintf(buffer + j, ",\"co2_accuracy\":%d",
                     processed_data.co2_accuracy);
        j += sprintf(buffer + j, ",\"iaq\":%.2f", processed_data.iaq);
        j += sprintf(buffer + j, ",\"iaq_accuracy\":%d",
                     processed_data.iaq_accuracy);
        j += sprintf(buffer + j, ",\"static_iaq\":%.2f",
                     processed_data.static_iaq);
        j += sprintf(buffer + j, ",\"static_iaq_accuracy\":%d",
                     processed_data.static_iaq_accuracy);
        j += sprintf(buffer + j, ",\"breath_voc_equivalent\":%.2f",
                     processed_data.breath_voc_equivalent);
        j += sprintf(buffer + j, ",\"breath_voc_accuracy\":%d",
                     processed_data.breath_voc_accuracy);
    ...
    bme68x_delay_us((sensor_settings.next_call - get_current_time_ns()) / 1000,
                    NULL);
  }
